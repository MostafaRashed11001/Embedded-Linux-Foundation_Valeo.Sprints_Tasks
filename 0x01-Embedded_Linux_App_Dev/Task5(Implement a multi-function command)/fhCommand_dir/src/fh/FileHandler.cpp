 ...
 * brief: A program that demonstrates file operations, shared memory, and semaphores.
 */
...
int main(void) {
    ...
    const char* PipeFileName = "../../Result/PipeChannel"; // Named pipe file name
    ...
    while (true) {
        // Receive operation code and extracted file path from the named pipe
        int receive_op = Rec_Pipe(PipeFileName, &fd);

        // Create shared memory and map it to virtual memory address
        if(!creatShared(MemoryName)) {
            LOG_ERROR << "[FileHandler] Failed to create shared memory";
        }

        // Create a binary semaphore with initial status "lock"
        if(!CreateSemaphore(semaphoreName, semaphoreState)) {
            LOG_ERROR << "[FileHandler] Failed to create semaphore";
        }
        
        if (receive_op == 0) {
            // If received option is 0 ("-r"), read and store file content
            string Dir_path = "";
            string File_Content = read_file_content((finalPath+3));  // Read file content
            if (File_Content == "") {
                LOG_WARNING << "[FileHandler] Empty file content";
            }
            memcpy(virtualMemoryAdd, File_Content.c_str(), memSize);  // Copy content to shared memory
            sem_post(semptr);  // Release the semaphore
        } else if (receive_op == 1) {
            // If received option is 1 ("-l"), list directory contents
            string File_path = "";
            string File_Content = list_dir((finalPath+3));  // List files in the directory
            if (File_Content == "") {
                LOG_WARNING << "[FileHandler] Empty Dir content";
            }
            memcpy(virtualMemoryAdd, File_Content.c_str(), memSize);  // Copy content to shared memory
            sem_post(semptr);  // Release the semaphore
        }
        ...
        if (sleep(1) != 0) {
            LOG_WARNING << "[FileHandler] Sleep interrupted: " << strerror(errno);
        }
        
        // Cleanup: deallocate memory, unlink shared memory, and close semaphore.
        delete[] finalPath;
